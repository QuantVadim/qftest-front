        ...
        <div class="p-inputtext">
            <table class="admin-table" v-if="selectedItems.length > 0">
                <tbody>
                    ...
                    <tr v-for="(item, index) in selectedItems" :key="item.usr_id" class="table-row" @click="ClickRow(item)" :class="{'__isremoved':isRemovedItem(item)}">
                        <td>{{index+1}}.</td>
                        <td class="td-user-name" >{{item.last_name+' '+item.first_name}}</td>
                    </tr>
                </tbody>
            </table>
...
<it-modal v-model="isModalItems">
  <template #header>
    <h3 style="margin: 10px 0px">Добвление пользователей</h3>
  </template>
  <template #body>

<div class="selected-adding-items">
    <span v-for="item in addingItems" :key="item.usr_id" @click="ClickSelectRow(item)">
    ...
    methods:{
        GetItems(){
            ...
            for (let i = 0; i < this.selectedItems.length; i++) {
                ...
                if(this.removedItems.find(itm=>itm.usr_id == el.usr_id) == undefined){
                    arr.push(el);
                }
        ...
        isHaveRow(item){
            let ret = false;
            if(this.selectedItems.find((itm)=>itm.usr_id == item.usr_id)){
                ret = true;
            }
            return ret;
        },
        isSelectedRow(item){
            let ret = false;
            if(this.addingItems.find((itm)=>itm.usr_id == item.usr_id)){
                ret = true;
            }
            return ret;
        },
